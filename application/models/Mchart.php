...
class Mchart extends CI_Model{
	...
	public function getregfac_by_year($y){
		$this->db->select("fname as 'faculty',(select count('user.idfac') from user inner join fac on user.idfac=fac.idfac and user.idrole='3' where DATE_FORMAT(user.ucreated,'%Y')='".$y."' and fname=faculty) as 'total'");
        $this->db->from('user');
        $this->db->where('idrole','3');
        $this->db->join('fac', 'user.idfac=fac.idfac','left');
        $this->db->group_by('user.idfac');
